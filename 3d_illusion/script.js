...
function updateCameraPosition(x, y, distance) {
    // Keep camera at fixed distance, only move laterally
    const baseDistance = 25;
    const cameraX = x * 0.05;
    const cameraY = y * 0.05;
    
    // Set camera position - Z stays constant
    camera.position.set(cameraX, cameraY, baseDistance);
    camera.lookAt(0, 0, 0);
    
    // Update room with subtle off-axis projection
    updateRoomProjection(x, y);
    
    // Update darts with depth movement based on distance
    updateDartPositions(x, y, distance);
}

// New function to update room projection (subtle effect)
function updateRoomProjection(offsetX, offsetY) {
    // Store original room projection for reference
    if (!room.userData.originalProjection) {
        room.userData.originalProjection = true;
    }
    
    // Apply subtle offset to room elements for background parallax
...
function updateDartPositions(offsetX, offsetY, distance) {
    targets.forEach((target, index) => {
        // Get original position stored during creation
        ...
        // Calculate parallax based on depth (Z position) for lateral movement
        const maxZ = -2;
        const minZ = -50;
        const normalizedDepth = (originalPos.z - minZ) / (maxZ - minZ);
        
        // Parallax multiplier: closer objects move more laterally
        const parallaxMultiplier = 1 + (normalizedDepth * 3);
        
        // Apply lateral position offset based on head movement and depth
        ...
        // NEW: Apply depth movement based on distance
        // Each dart moves forward/backward based on its original depth and user distance
        const depthScale = 0.08; // Adjust this to control sensitivity
        const depthMovement = distance * depthScale;
        
        // Closer darts (higher Z values) move more dramatically
        const depthMultiplier = Math.max(0.1, 1 - Math.abs(originalPos.z) / 50);
        const newDartZ = originalPos.z + (depthMovement * depthMultiplier);
        
        // Update dart position with depth movement
        target.position.set(newDartX, newDartY, newDartZ);
        target.lookAt(camera.position.x, camera.position.y, 25);
        
        // Update the corresponding line for this dart
        if (dartLines[index]) {
            const dartLine = dartLines[index];
            const userData = dartLine.userData;
            
            // Update line geometry
            // Start point moves with the dart (with parallax AND depth)
            const startX = newDartX;
            const startY = newDartY;
            const startZ = newDartZ;
            
            // End point stays fixed at the original position behind the dart
...
function updateFOVZoom(distance) {
    ...
    // Calculate zoom factor - positive distance zooms in, negative zooms out
    const zoomFactor = Math.max(0.3, Math.min(2.0, 1 + (distance * 0.001)));
    
    // Adjust FOV inversely - smaller FOV = more zoomed in
...
function updateTargetZoom(distance) {
    // Calculate zoom factor for darts only
    const zoomFactor = Math.max(0.5, Math.min(2.0, 1 + (distance * 0.003)));
    
    // Apply zoom to all dart targets
...
function updateOffAxisProjection(offsetX, offsetY) {
    ...
    const fov = camera.fov; // Use current FOV
    const near = 0.1;
    const far = 1000;
    ...
    // Offset scaling remains the same
    ...
    camera.projectionMatrix.makePerspective(
        ...
        function setup3DScene() {
            ...
            scene.fog = new THREE.Fog(0x000000, 30, 200); // Add fog for infinite depth effect
        ...
        function createInfiniteRoom() {
            ...
            // Extended room dimensions for infinite effect
            const width = 150;
            const height = 100;
            const depth = 200;
            const gridSpacing = 8;
            
            // Create material with distance-based opacity
            ...
            // Create floor grid extending into distance
...
// Add global array to track line segments
let dartLines = [];
// Modified createTargets function with perpendicular lines to individual fixed points
function createTargets() {
    ...
    positions.forEach((pos, index) => {
        ...
        // Fixed scale for all targets (no distance-based scaling)
        const scale = 1.0;

        // Calculate darkening factor based on distance
        const maxDistance = 50;
        const currentDistance = Math.abs(pos.z);
        const darkenFactor = Math.min(currentDistance / maxDistance, 0.6);

        // Create concentric dartboard rings with uniform size and distance-based darkening
        const ringData = [
        ...
        // Add center dot with distance-based darkening
        ...
        // Store original position in userData for parallax calculations
        ...
        // Calculate the fixed endpoint for this dart's line
        // Each line extends straight back from its dart position
        const lineLength = 80;
        const fixedEndpointZ = pos.z - lineLength;
        
        // Create single perpendicular line from dart extending straight back
        const lineGeometry = new THREE.BufferGeometry();
        const linePoints = [
            pos.x, pos.y, pos.z,              // Start at dart position
            pos.x, pos.y, fixedEndpointZ      // End at fixed point behind dart
        ];
        ...
        // Store single line for this dart
...
// Function to generate unique anchor points for each dart
function generateAnchorPoints(numDarts, backPlaneZ) {
    const anchorPoints = [];
    
    // Create a distribution pattern for anchor points
    // Using a spiral or grid pattern to spread them out
    const gridSize = Math.ceil(Math.sqrt(numDarts));
    const spacing = 20; // Spacing between anchor points
...
// Modified resetTracking function
...
function resetTracking() {
    ...
    camera.position.set(0, 0, 25);
    camera.lookAt(0, 0, 0);
    // Remove FOV reset since we're not using FOV zoom anymore
    ...
    // Reset dart positions to original
    targets.forEach((target, index) => {
        if (target.userData.originalPosition) {
            const originalPos = target.userData.originalPosition;
        ...
        // Reset line to original positions
        if (dartLines[index]) {
            const dartLine = dartLines[index];
            const userData = dartLine.userData;
